...
long long **user_graph;
long long *friend_size;
char **user;
int binary = 0, cbow = 1, debug_mode = 2, window = 5, min_count = 5, num_threads = 12, min_reduce = 1, max_path_length = 20, average_num_paths = 40, path_window = 1;
int *vocab_hash;
int *user_hash;
real **transition_probs;
long long **transition_aliases;
int *num_paths;
real *restart_alpha;
long long vocab_max_size = 1000, user_max_size = 1000, user_graph_max_size = 1000, vocab_size = 0, user_size = 0, layer1_size = 100;
long long train_words = 0, word_count_actual = 0, iter = 5, file_size = 0, classes = 0, friend_count = 0;
real alpha = 0.025, lambda = 8, starting_alpha, sample = 1e-3, r = 0.25, average_restart_alpha = 0.02, p = 1, q = 1, bias_weight = 0.2;
real l2norm_square_lowerbound, l2norm_square_upperbound, average_friend_count;
real *syn0, *syn1, *syn1neg, *user0, *user1neg, *expTable;
clock_t start;
int hs = 0, negative = 5;
...
int UserIDCompare (const void * a, const void * b);
void LearnVocabFromTrainFile();
void LearnUserFromUserFile();
void CreateBinaryTree();
void CreateAdjacentList();
void AliasSetup(long long src, long long dst, real *probs, long long probs_length);
long long AliasSample(long long src, long long dst, unsigned long long *next_random);
void PreprocessTransitionEdgeProbs(long long src, long long dst);
...
int Node2VecWalk(long long *path, long long start, unsigned long long *next_random);
void Socialization(long long *path, int path_length);
void TrainModel();
...
void CreateAdjacentList(){
    char word[MAX_STRING];
    FILE *fin;
    long long i, head, friend;
    user_graph = (long long **)malloc(user_size * sizeof(long long *));
    friend_size = (long long *)calloc(user_size, sizeof(long long));
    ...
    while(1) {
        ...
        ReadWord(word, fin);
        friend_size[head] = atoll(word);
        user_graph[head] = (long long*)malloc(friend_size[head] * sizeof(long long));
        i = 0;
        while (i < friend_size[head]) {
            ReadWord(word, fin);
            friend = SearchUser(word);
            if (friend == -1) {
                --friend_size[head];
                continue;
            }
            user_graph[head][i++] = friend;
        }
        qsort(user_graph[head], friend_size[head], sizeof(long long), UserIDCompare);
        friend_count += friend_size[head];
    }
    fclose(fin);
}

void AliasSetup(long long src, long long dst, real *probs, long long probs_length) {
    long long smaller_size = 0, larger_size = 0, i, small, large;
    long long *J = transition_aliases[src * user_size + dst] = (long long *)calloc(probs_length, sizeof(long long));
    if (J == NULL) {
        printf("Not enough memory for transition_probs");
        exit(1);
    }
    real *q = transition_probs[src * user_size + dst] = (real *)malloc(probs_length * sizeof(real));
    if (q == NULL) {
        printf("Not enough memory for transition_aliases");
        exit(1);
    }
    long long *smaller = (long long *)malloc(probs_length * sizeof(long long));
    long long *larger = (long long *)malloc(probs_length * sizeof(long long));
    if (smaller == NULL || larger == NULL) {
        printf("Not enough memory for auxiliary arrays");
        exit(1);
    }
    for (i = 0; i < probs_length; ++i) {
        q[i] = probs[i] * probs_length;
        if (q[i] < 1.0) {
            smaller[smaller_size++] = i;
        } else {
            larger[larger_size++] = i;
        }
    }
    while (smaller_size > 0 && larger_size > 0) {
        small = smaller[--smaller_size];
        large = larger[--larger_size];
        J[small] = large;
        q[large] = q[large] - (1.0 - q[small]);
        if (q[large] < 1.0) {
            smaller[smaller_size++] = large;
        } else {
            larger[larger_size++] = large;
        }
    }
    free(smaller);
    free(larger);
...
long long AliasSample(long long src, long long dst, unsigned long long *next_random) {
    if (friend_size[dst] == 0) return -1;
    long long *J = transition_aliases[src * user_size + dst];
    real *q = transition_probs[src * user_size + dst];
    if (J == NULL || q == NULL) return AliasSample(dst, dst, next_random);
    long long i;
    real ran;
    (*next_random) = (*next_random) * (unsigned long long)25214903917 + 11;
    i = (*next_random) % friend_size[dst];
    (*next_random) = (*next_random) * (unsigned long long)25214903917 + 11;
    ran = (((*next_random) & 0xFFFF) / (real)65536);
    if (ran < q[i]) return user_graph[dst][i];
    else return user_graph[dst][J[i]];
}

void PreprocessTransitionEdgeProbs(long long src, long long dst) {
    if (friend_size[dst] == 0) return;
    long long i, friend;
    real *probs = (real *)malloc(friend_size[dst] * sizeof(real));
    real sum = 0;
    if (probs == NULL){
        printf("Not enough memory for probs");
        exit(1);
    }
    for (i = 0; i < friend_size[dst]; ++i) {
        friend = user_graph[dst][i];
        if (friend == src) {
            probs[i] = BiasCompute(1.0, bias_weight, friend) / p;
        } else if ((long long *)bsearch(&friend, user_graph[src], friend_size[src], sizeof(long long), UserIDCompare) != NULL) {
            probs[i] = BiasCompute(1.0, bias_weight, friend);
        } else {
            probs[i] = BiasCompute(1.0, bias_weight, friend) / q;
        }
        sum += probs[i];
    }
    for (i = 0; i < friend_size[dst]; ++i) probs[i] /= sum;
    AliasSetup(src, dst, probs, friend_size[dst]);
    free(probs);
}

void PreprocessTransitionProbs() {
    long long user, i;
    real *probs = (real *)malloc(user_size * sizeof(real));
    real sum = 0;
    if (probs == NULL) {
        printf("Not enough memory for probs");
        exit(1);
    }
    transition_probs = (real **)calloc(user_size * user_size, sizeof(real *));
    if (transition_probs == NULL) {
        printf("Not enough memory for transition_probs");
        exit(1);
    }
    transition_aliases = (long long **)calloc(user_size * user_size, sizeof(long long *));
    if (transition_aliases == NULL) {
        printf("Not enough memory for transition_aliases");
        exit(1);
    }
    num_paths = (int *)calloc(user_size, sizeof(int));
    if (num_paths == NULL) {
        printf("Not enough memory for num_paths");
        exit(1);
    }
    restart_alpha = (real *)calloc(user_size, sizeof(real));
    if (restart_alpha == NULL) {
        printf("Not enough memory for restart_alpha");
        exit(1);
    }
    average_friend_count = friend_count / (real)user_size;
    for (user = 0; user < user_size; ++user) {
        if (friend_size[user] == 0) continue;
        num_paths[user] = BiasCompute(average_num_paths, bias_weight, user);
        restart_alpha[user] = BiasCompute(average_restart_alpha, bias_weight, user);;
        for (i = 0; i < friend_size[user]; ++i) {
            probs[i] = BiasCompute(1.0, bias_weight, user);
            sum += probs[i];
            PreprocessTransitionEdgeProbs(user, user_graph[user][i]);
        }
        for (i = 0; i < friend_size[user]; ++i) probs[i] /= sum;
        AliasSetup(user, user, probs, friend_size[user]);
    }
    free(probs);
}
...
void InitNet() {
    ...
    if (negative>0) {
        ...
        a = posix_memalign((void **)&user1neg, 128, (long long)user_size * layer1_size * sizeof(real));
        if (user1neg == NULL) {printf("Memory allocation failed\n"); exit(1);}
        for (a = 0; a < user_size; a++) for (b = 0; b < layer1_size; b++)
            user1neg[a * layer1_size + b] = 0;
...
void *TrainModelThread(void *id) {
    ...
    long long a, b, c, d, cw, word, last_word, sentence_length = 0, sentence_position = 0, user_id, unknow_id;
    long long word_count = 0, last_word_count = 0, sen[MAX_SENTENCE_LENGTH + 1];
    long long l1, l2, target, label, local_iter = iter;  
    unsigned long long next_random = (long long)id;
    long long *path = (long long*)malloc(max_path_length*sizeof(long long));
    clock_t now;
    ...
    while (1) {
            ...
            if (cw) { //cw is the number of context words of the target word
                ...
                if (unknow != 1) {
                    ...
                    for(c = 0; c < layer1_size; c++) {
                        user0[user_id * layer1_size + c] += neu1e[c];
                        l2norm_square1 += user0[user_id * layer1_size + c] * user0[user_id * layer1_size + c];
                    }
                    while (1) {
                        if (l2norm_square1 < l2norm_square_lowerbound) {
                            for(c = 0; c < layer1_size; c++) {
                                user0[user_id * layer1_size + c] *= 1.1;
                            }
                            l2norm_square1 *= 1.21;
                        } else if (l2norm_square1 > l2norm_square_upperbound) {
                            for(c = 0; c < layer1_size; c++) {
                                user0[user_id * layer1_size + c] *= 0.9;
                            }
                            l2norm_square1 *= 0.81;
                        } else {
                            break;
        ...
        if (sentence_position >= sentence_length) {
            sentence_length = 0;
            if (unknow != 1){
                if (cut == 0 && friend_size[user_id] > 0) {
                    for (a = 0; a < num_paths[user_id]; ++a) {
                        // generate randomwalks
                        path_length = Node2VecWalk(path, user_id, &next_random);
                        Socialization(path, path_length);
                    }
                }
            }
            continue;
...
int Node2VecWalk(long long *path, long long start, unsigned long long *next_random) {
    ...
    real ran;
    int i;
    if (friend_size[start] == 0) return 0;
    path[0] = start;
    for (i = 1; i < max_path_length; ++i) {
        (*next_random) = (*next_random) * (unsigned long long)25214903917 + 11;
        ran = (((*next_random) & 0xFFFF) / (real)65536);
        if (ran < restart_alpha[start]) {
            path[i] = start;
        } else {
            cur = path[i - 1];
            pre = i == 1 ? cur : path[i - 2];
            path[i] = AliasSample(pre, cur, next_random);
            if (path[i] == -1) break;
        }
    }
    return i;
...
void Socialization(long long *path, int path_length) {
    long long a, b, c, user_id, friend_id;
    real l2norm_square1, l2norm_square2, temp;
    for (a = 0; a < path_length; ++a) {
        user_id = path[a];
        for (b = a - path_window; b <= a + path_window; ++b) {
            if (b == a || b < 0 || b >= path_length) continue;
            friend_id = path[b];
            if (user_id == friend_id) continue;
            l2norm_square1 = 0;
            l2norm_square2 = 0;
            for(c = 0; c < layer1_size; c++) {
                temp = alpha * lambda * (user0[user_id * layer1_size + c] - user0[friend_id * layer1_size + c]);
                user0[user_id * layer1_size+c] -= temp;
                user0[friend_id * layer1_size+c] += temp;
                l2norm_square1 += user0[user_id*layer1_size+c] * user0[user_id*layer1_size+c];
                l2norm_square2 += user0[friend_id*layer1_size+c] * user0[friend_id*layer1_size+c];
            }
            while (1) {
                if (l2norm_square1 < l2norm_square_lowerbound) {
                    for(c = 0; c < layer1_size; c++) {
            ...
            while (1) {
                if (l2norm_square2 < l2norm_square_lowerbound) {
                    for(c = 0; c < layer1_size; c++) {
                        user0[friend_id * layer1_size + c] *= 1.1;
                    }
                    l2norm_square2 *= 1.21;
                } else if (l2norm_square2 > l2norm_square_upperbound) {
                    for(c = 0; c < layer1_size; c++) {
                        user0[friend_id * layer1_size + c] *= 0.9;
                    }
                    l2norm_square2 *= 0.81;
                } else {
                    break;
...
void TrainModel() {
    ...
    l2norm_square_upperbound = r * r;
    l2norm_square_lowerbound = l2norm_square_upperbound * 0.2;
...
int main(int argc, char **argv) {
    ...
    if ((i = ArgPos((char *)"-paths", argc, argv)) > 0) average_num_paths = atoi(argv[i + 1]);
    if ((i = ArgPos((char *)"-path-length", argc, argv)) > 0) max_path_length = atoi(argv[i + 1]);
    if ((i = ArgPos((char *)"-path-window", argc, argv)) > 0) path_window = atoi(argv[i + 1]);
    if ((i = ArgPos((char *)"-p", argc, argv)) > 0) p = atof(argv[i + 1]);
    if ((i = ArgPos((char *)"-q", argc, argv)) > 0) q = atof(argv[i + 1]);
    if ((i = ArgPos((char *)"-restart-alpha", argc, argv)) > 0) average_restart_alpha = atof(argv[i + 1]);
    if ((i = ArgPos((char *)"-bias-weight", argc, argv)) > 0) bias_weight = atof(argv[i + 1]);
