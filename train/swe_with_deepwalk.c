...
real alpha = 0.025, lambda = 8, starting_alpha, sample = 1e-3, r = 0.25, randomwalk_alpha = 0;
...
int RandomWalk(long long *path, long long start, unsigned long long *next_random);
            ...
            if (unknow != 1){
                if (cut == 0) {
                    for (a = 0; a < num_paths; ++a) {
                        ...
                        path_length = RandomWalk(path, user_id, &next_random);
...
int RandomWalk(long long *path, long long start, unsigned long long *next_random) {
    ...
    for (i = 1; i < max_path_length; ++i) {
        cur = path[i-1];
        if (friend_size[cur] == 0) break;
        (*next_random) = (*next_random) * (unsigned long long)25214903917 + 11;
        path[i] = user_graph[cur][(*next_random) % (friend_size[cur])];
    }
    return i;
}
