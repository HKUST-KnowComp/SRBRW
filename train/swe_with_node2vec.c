...
void SkipGram(long long *path, int path_length, unsigned long long *next_random);
void TrainModel();
    ...
    while (1) {
        ...
        if (cbow) {  //train the cbow architecture
            ...
            if (cw) { //cw is the number of context words of the target word
                ...
                //         l2norm_square += user0[user_id * layer1_size + c] * user0[user_id * layer1_size + c];
                //     }
                //     while (1) {
                //         if (l2norm_square < l2norm_square_lowerbound) {
                //             for(c = 0; c < layer1_size; c++) {
                //                 user0[user_id * layer1_size + c] *= 1.1;
                //             }
                //             l2norm_square *= 1.21;
                //         } else if (l2norm_square > l2norm_square_upperbound) {
                //             for(c = 0; c < layer1_size; c++) {
            ...
            if (unknow != 1){
                if (cut == 0 && friend_size[user_id] > 0) {
                    for (a = 0; a < num_paths; ++a) {
                        ...
                        SkipGram(path, path_length, &next_random);
                    }
                }
            }
            continue;
...
void SkipGram(long long *path, int path_length, unsigned long long *next_random) {
    long long a, b, c, d, user_id, last_user;
    long long l1, l2, target, label;
    real f, g, l2norm_square;
    real *neu1e = (real *)calloc(layer1_size, sizeof(real));
        ...
        for (b = a - path_window; b <= a + path_window; ++b) {
            ...
            if (negative > 0) for (d = 0; d < negative + 1; d++) {
                if (d == 0) {
                    ...
                    (*next_random) = (*next_random) * (unsigned long long)25214903917 + 11;
                    target = (*next_random) % (user_size - 1) + 1;
                    if (target == user_id) continue;
                    label = 0;
                }
                l2 = target * layer1_size;
                f = 0;
                for (c = 0; c < layer1_size; c++) f += user0[c + l1] * user1neg[c + l2];
                if (f > MAX_EXP) g = (label - 1) * alpha;
                else if (f < -MAX_EXP) g = (label - 0) * alpha;
                else g = (label - expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]) * alpha;
                for (c = 0; c < layer1_size; c++) neu1e[c] += g * user1neg[c + l2];
                for (c = 0; c < layer1_size; c++) user1neg[c + l2] += g * user0[c + l1];
            }
            // Learn weights input -> hidden
            l2norm_square = 0;
            for (c = 0; c < layer1_size; c++) {
                user0[c + l1] += neu1e[c];
                l2norm_square += user0[c + l1] * user0[c + l1];
            }
            while (1) {
                if (l2norm_square < l2norm_square_lowerbound) {
                    for(c = 0; c < layer1_size; c++) {
