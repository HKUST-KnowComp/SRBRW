...
def create_optimization_updates(
    ...
    if max_norm is not None and max_norm is not False:
        max_norm = theano.shared(np.float64(
            max_norm).astype(theano.config.floatX))
    ...
    if method == "sgd":
        ...
        create_adadelta_updates(updates, params, gparams,
                                gsums, xsums, lr, eps, rho)

    elif method == "adam":
        create_adam_updates(updates, params, gparams, gsums,
                            xsums, lr, eps, beta1, beta2)

    elif method == "esgd":
        create_esgd_updates(updates, params, gparams, gsums,
                            xsums, lr, eps, gamma, momentum)
